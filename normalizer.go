...
import (
	...
	"github.com/houkx/sqlparser/dependency/sqltypes"

	"github.com/houkx/sqlparser/dependency/querypb"
)
...
func Normalize(stmt Statement, bindVars map[string]*querypb.BindVariable, prefix string) {
	...
	_ = Walk(0, nz.WalkStatement, stmt)
}
...
func (nz *normalizer) WalkStatement(level int, node SQLNode) (bool, error) {
	switch node := node.(type) {
	case *Select:
		_ = Walk(0, nz.WalkSelect, node)
		// Don't continue
		return false, nil
	case *SQLVal:
...
func (nz *normalizer) WalkSelect(level int,node SQLNode) (bool, error) {
	switch node := node.(type) {
	case *SQLVal:
...
func GetBindvars(stmt Statement) map[string]struct{} {
	...
	_ = Walk(0, func(level int, node SQLNode) (kontinue bool, err error) {
		switch node := node.(type) {
		case *SQLVal:
			if node.Type == ValArg {
